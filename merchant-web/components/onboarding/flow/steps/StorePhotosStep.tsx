...
import { mediaService, type MediaUploadPreset } from "@/lib/api/media.service";
...
export function StorePhotosStep({ draft, onChange }: Props) {
  ...
  const applyCrop = async (blob: Blob) => {
    ...
    try {
      ...
      const url = await mediaService.uploadMedia(uploadFile, uploadPresetByTarget[pendingTarget]);
