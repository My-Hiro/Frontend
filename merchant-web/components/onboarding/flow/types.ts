import type { Category, StoreOpenHour, StoreProfile } from "@/types";
  ...
  return {
    ...
    openHoursByDay:
      Array.isArray(profile.openHoursByDay) && profile.openHoursByDay.length > 0
        ? profile.openHoursByDay.map((entry: StoreOpenHour) => ({ ...entry }))
...
export const applyOnboardingDraftToProfile = (
  ...
  return {
    ...
    lat: draft.lat,
    lng: draft.lng,
    openHoursByDay: draft.openHoursByDay.map((entry: StoreOpenHour) => ({ ...entry })),
    openHours,
