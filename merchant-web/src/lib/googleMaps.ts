...
export const loadGoogleMapsJs = async (): Promise<any> => {
  ...
  mapsLoader = new Promise((resolve, reject) => {
    const existing = document.querySelector<HTMLScriptElement>("script[data-merchant-google-maps='1']");
    if (existing) {
      existing.addEventListener("load", () => resolve(window.google?.maps));
      existing.addEventListener("error", () => reject(new Error("Could not load Google Maps script.")));
      return;
    }
    ...
    script.async = true;
    script.defer = true;
    script.dataset.merchantGoogleMaps = "1";
