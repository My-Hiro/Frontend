...
export function ImageCropModal({
  ...
  const exportCrop = async () => {
    ...
    try {
      ...
      const centerX = imageNatural.width / 2 + clamp(offsetX / 100, -1, 1) * maxOffsetX;
      const centerY = imageNatural.height / 2 + clamp(offsetY / 100, -1, 1) * maxOffsetY;
    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Adjust image crop">
      <div className="modal modal-wide">
            ...
            {sourceUrl ? (
              <img
                ...
                onLoad={(event) => {
                  ...
                  setImageNatural({ width: target.naturalWidth, height: target.naturalHeight });
                }}
              />
            ) : (
        ...
        <footer>
          ...
          <button type="button" className="action primary" onClick={() => void exportCrop()} disabled={busy}>
