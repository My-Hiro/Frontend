...
const getFallbackHomeData = (): {
  ...
  const popularProducts = fallbackStoreSeed
    .flatMap((store: any) => store.products.map((product: any) => toFallbackProductCard(store, product)))
    .slice(0, 24);
  const sections = fallbackPlatformCategories
    .map((category) => ({
      ...
      stores: sortStoresByDistance(
        fallbackStoreSeed
          .filter((store: any) => resolveFallbackStoreCategoryId(store) === category.id)
          .map((store: any) => toFallbackStoreCard(store))
      )
    }))
...
const findFallbackStore = (storeId: string): FallbackStore | undefined =>
  fallbackStoreSeed.find((store: any) => store.id === storeId);

const getFallbackCategoryStores = (categoryId: string): StoreCardModel[] =>
  sortStoresByDistance(
    fallbackStoreSeed
      .filter((store: any) => resolveFallbackStoreCategoryId(store) === categoryId)
      .map((store: any) => toFallbackStoreCard(store))
  );
...
const getFallbackStoreProducts = (storeId: string): ProductCardModel[] => {
  ...
  return fallbackStore.products.map((product: any) => toFallbackProductCard(fallbackStore, product));
};
...
const getFallbackProduct = (
  ...
  const matches = fallbackStoreSeed.flatMap((store: any) =>
    store.products
      .filter((product: any) => product.id === productId)
      .map((product: any) => ({ store, product }))
  );

  if (matches.length === 0) return null;

  const primary =
    matches.find((entry: any) => entry.store.id === preferredStoreId) ??
    matches[0];
  ...
  const alsoAvailableAt = matches
    .filter((entry: any) => entry.store.id !== primary.store.id)
    .map((entry: any) => ({
