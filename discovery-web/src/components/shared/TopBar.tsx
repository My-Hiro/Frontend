...
import { Search, MapPin, Tag, ChevronLeft } from "lucide-react";
...
export function TopBar({
  ...
  const location = useDiscoveryStore((state) => state.location);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between px-4 lg:px-6 py-2">
          <div className="hidden md:flex lg:hidden items-center gap-2 mr-4">
            <span className="text-lg text-primary font-bold">myHiro</span>
          </div>
          <button
            className="flex items-center gap-1.5 text-sm text-foreground min-h-[36px]"
            aria-label={`Current location: ${location.label}`}
            type="button"
