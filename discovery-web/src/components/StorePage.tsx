...
import { StorePageSkeleton } from "./SkeletonLoaders";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [cats, storeRaw, storeProducts, banner] = await Promise.all([
        ...
        discoveryApi.getStore(storeId),
        discoveryApi.getStoreProducts(storeId),
        discoveryApi.getSponsoredBanners("store_page")
      ]);
      if (!mounted) return;
