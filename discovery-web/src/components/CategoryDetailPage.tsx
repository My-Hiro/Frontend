...
import type { PlatformCategory, StoreCardModel } from "../state/types";
    ...
    const load = async () => {
      try {
        ...
        const [cats, categoryStores] = await Promise.all([
          discoveryApi.getPlatformCategories(),
          discoveryApi.getCategoryStores(categoryId),
        ]);
        if (!mounted) return;
        setCategories(cats);
    ...
    <div className="pb-20 lg:pb-8 max-w-7xl mx-auto">
      ...
      <div className="px-4 lg:px-6 pt-4">
        {error ? (
          ...
          <EmptyState type="empty-category" onRetry={onBack} />
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              ...
              <StoreCard
                ...
                onPress={(storeId) => {
                  void discoveryApi
                    ...
                    .catch(() => undefined);
                  onStorePress(storeId, categoryId);
                }}
              />
            ))}
          </div>
