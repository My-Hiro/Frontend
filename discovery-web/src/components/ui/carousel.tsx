...
function CarouselContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const handleWheel = React.useCallback(
    (event: React.WheelEvent<HTMLDivElement>) => {
      ...
      // Treat trackpad horizontal scroll as intent. Support Shift+wheel as a fallback.
      let direction: "next" | "prev" | null = null;
      if (absX > 2) {
        direction = event.deltaX > 0 ? "next" : "prev";
      } else if (event.shiftKey && absY > 2) {
      ...
      if (api.canScrollPrev()) {
        ...
        api.scrollTo(Math.max(0, snapCount - 1), true);
      }
    },
    [api, orientation],
  );
  ...
  return (
    ...
    >
      <div
        className={cn(
          "embla-track",
          orientation === "vertical" ? "embla-track--vertical" : "",
          className,
        )}
        {...props}
      />
