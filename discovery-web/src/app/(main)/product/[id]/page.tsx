...
import { BannerRotator } from "@/components/features/BannerRotator";
...
import type { HeroBannerModel, ProductCardModel, StoreCardModel } from "@/types";
import {
  ArrowLeft,
  Phone,
  Navigation,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [result, banner] = await Promise.all([
        discoveryService.getProduct(productId),
        discoveryService.getSponsoredBanners("product_page")
      ]);
      ...
      if (result.product.storeId) {
        try {
          const storeDetail = await discoveryService.getStore(result.product.storeId);
          if (!mounted) return;
            ...
            <div className="mx-4 lg:mx-0 mt-3 bg-card rounded-xl border border-border p-4">
              ...
              <div className="space-y-2">
                {alsoAvailableAt.map((entry) => (
                  <button
                    key={entry.storeId}
                    onClick={() => router.push(`/store/${entry.storeId}`)}
